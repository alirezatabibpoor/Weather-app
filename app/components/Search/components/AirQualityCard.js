...
export default function AirQualityCard({ air, extra }) {
  if (!air || !extra?.hourly) return null;

  const components = air.list?.[0]?.components;

  if (!components) return null;

  // نزدیک‌ترین ساعت به زمان فعلی
  const now = Date.now();

  let currentIndex = 0;
  let minDiff = Infinity;

  extra.hourly.time.forEach((time, index) => {
  ...
  const aqi = extra.hourly.us_aqi?.[currentIndex] ?? null;
    ...
    >
      ...
      >
        <div className="flex items-center justify-between">
          ...
          `}
          >
            {aqi ? getAQIStatus(aqi) : "No Data"}
          </div>
        </div>

        <div className="mt-5">
          <Progress
            percent={getAQIPercent(aqi)}
